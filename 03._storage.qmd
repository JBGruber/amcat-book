...
```{r, eval=FALSE}
library(amcat4r)
amcat_auth("http://localhost/api", username = "admin", password = "supergeheim")
```

```{r, echo=FALSE}
library(amcat4r)
amcat_auth("http://localhost/api", username = "admin", password = "supergeheim", force_refresh = TRUE, cache = 1L)
```
