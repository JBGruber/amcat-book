...
Head over to the Docker website to get [Docker Desktop](https://docs.docker.com/get-docker/) or the [Docker Engine](https://docs.docker.com/engine/install/) and [Docker Compose](https://docs.docker.com/compose/install/) to use Docker from the command line.
...
You can get it from [here](https://raw.githubusercontent.com/JBGruber/amcat4docker/main/docker-compose.yml) (save the file as `docker-compose.yml`).
...
- In lines `r grep("volumes:", docker_compose):(grep("volumes:", docker_compose) + 1)` we suggested a setting so Elasticsearch will store your data on a volume on the host machine. We recommend this to make it easier to back up your database and reuse it with a different installation of Elasticsearch (e.g., after an update) in the future. However, the container will not run if it does not have proper access to this folder. See the comment to solve this. Note that the suggested local path is just an example.
:::
...
If you are using Docker Desktop, you can also monitor the containers there:
...
If you decide not to go with Docker, for example, because you have your own server and you need more control over, you can also run amcat on your system directly.
...
::: {.callout-note}
If you are using amcat on our servers or through docker, you can skip this section and move on to install an API client to start managing amcat

either the [R](#amcat4r) or [Python](#amcat4apiclient) client to start managing amcat.
:::
